 ...
 function inlineFormat(str: string): React.ReactNode[] {
  ...
  while ((m = re.exec(str)) !== null) {
   ...
   if (m[2]) out.push(<strong key={idx++}>{m[2]}</strong>);          // **bold**
    ...
    out.push(<a key={idx++} href={href} target="_blank" rel="noopener" style={{ color: '#3498db' }}>{m[5]}</a>);
   }
  ...
  <Box sx={{ p: { xs: 1.5, sm: 2.5 }, maxWidth: 1200, mx: 'auto' }}>
   ...
   <Grid container spacing={2} sx={{ mb: 2 }}>
    <Grid item xs={12} md={6}>
     <Paper sx={{ height: '100%', overflow: 'hidden', background: 'linear-gradient(135deg, #3498db12, #0e72b508)', border: '1px solid', borderColor: 'divider' }}>
      <Box sx={{ p: 2, display: 'flex', alignItems: 'center', gap: 1.5 }}>
       <SmartToy sx={{ color: '#3498db', fontSize: 28 }} />
       <Box sx={{ flex: 1 }}>
        <Typography sx={{ fontWeight: 700, fontSize: 16 }}>LinkedIn Powerhouse AI Assistant</Typography>
        <Typography variant="caption" color="text.secondary">Leverage AI-powered tools to maximize your LinkedIn impact.</Typography>
       </Box>
       {chatResult && (
        <Tooltip title="Clear"><IconButton size="small" onClick={() => { setChatResult(null); setChatProgress([]); setChatInput(''); }}><Delete sx={{ fontSize: 18 }} /></IconButton></Tooltip>
       )}
      </Box>
      <Box sx={{ px: 2, pb: 2, display: 'flex', gap: 1, alignItems: 'center' }}>
       <TextField inputRef={chatInputRef} size="small" fullWidth placeholder="Ask the AI anything — research, create content, generate documents..."
     ...
     <Grid container spacing={1.5}>
      {activeSkills.map(s => (
       <Grid item xs={12} sm={6} md={4} key={s.id}>
        <Card variant="outlined" sx={{ height: '100%', border: selectedId === s.id ? '2px solid #3498db' : undefined, transition: 'border 0.15s' }}>
