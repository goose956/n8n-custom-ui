...
export function MembersDocumentsPage() {
 ...
 const load = useCallback(async () => {
  ...
  try {
   ...
   const res = await fetch(API_BASE + '/api/skills/files' + cat).then(r => r.json());
   setFiles(res.files || []);
 ...
 const del = async (f: FileEntry) => {
  setDelTarget(null);
  try {
   const res = await fetch(API_BASE + '/api/skills/files/' + f.category + '/' + encodeURIComponent(f.name), { method: 'DELETE' }).then(r => r.json());
   if (!res.success) throw new Error(res.message);
    ...
    <Box sx={{ position: 'relative', zIndex: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
     <Box>
      <Typography variant="h4" sx={{ fontWeight: 800, display: 'flex', alignItems: 'center', gap: 1 }}>
       <Folder /> LinkedIn Powerhouse Documents
      </Typography>
      <Typography sx={{ opacity: 0.85, mt: 0.5 }}>Manage templates and outputs from your LinkedIn activities.</Typography>
     </Box>
