...
export function MembersContactPage() {
 ...
 const handleSubmit = async (e: React.FormEvent) => {
  ...
  try {
   const res = await fetch(API + '/api/contact', {
    ...
    body: JSON.stringify({ ...form, app_id: 11 }),
   });
 ...
 const infoCards = [
  { icon: <Chat />, title: 'Customer Support', desc: 'Available 9 AM - 5 PM EST, Monday to Friday', color: COLORS.primary },
  { icon: <AccessTime />, title: 'Email Support', desc: 'Response time within 24 hours', color: COLORS.blue },
  { icon: <SupportAgent />, title: 'Live Chat', desc: 'Instant responses from our team during business hours', color: COLORS.success },
 ];
  ...
  <Box sx={{ maxWidth: 800, mx: 'auto' }}>
   <Paper sx={heroSx}>
    ...
    <Box sx={{ position: 'relative', zIndex: 1 }}>
     ...
     <Typography variant="body1" sx={{ opacity: 0.85, mt: 0.5 }}>We’re here to help! Reach out with any questions or feedback.</Typography>
    </Box>
   </Paper>
