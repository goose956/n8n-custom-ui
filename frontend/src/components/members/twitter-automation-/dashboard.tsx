...
export function MembersDashboardPage() {
 const stats = [
  { label: 'Daily Tweets Scheduled', value: '25', change: '+10%', icon: <Visibility />, color: COLORS.primary },
  { label: 'Engagement Rate', value: '3.5%', change: '+0.5%', icon: <People />, color: COLORS.blue },
  { label: 'New Followers Gained', value: '50', change: '+15%', icon: <TrendingUp />, color: COLORS.success },
  { label: 'Opt-in Conversions', value: '200', change: 'Top 20%', icon: <Star />, color: COLORS.warning },
 ];

 const recentActivity = [
  { title: 'Latest Tweets Generated', desc: 'View the most recently created tweets in your list.', time: '2 min ago', color: COLORS.success, icon: <People /> },
  { title: 'Engagement Responses', desc: 'See how many replies you’ve automatically addressed today.', time: '1 hour ago', color: COLORS.warning, icon: <EmojiEvents /> },
  { title: 'Content Scraping Completion', desc: 'Check the latest content scraped from your chosen sources.', time: '3 hours ago', color: COLORS.primary, icon: <Bolt /> },
  { title: 'New Opt-ins', desc: 'Track the latest opt-in conversions for your ebook.', time: '5 hours ago', color: COLORS.blue, icon: <Star /> },
 ];
  ...
  <Box>
   <Paper sx={heroSx}>
    ...
    <Box sx={{ position: 'relative', zIndex: 1 }}>
     ...
     <Typography variant="body1" sx={{ opacity: 0.85, maxWidth: 500 }}>Effortlessly manage your Twitter engagement and content strategy.</Typography>
    </Box>
   </Paper>
    ...
    <Grid item xs={12} md={4}>
     ...
     <Paper sx={{ ...sectionSx, mt: 3, background: 'linear-gradient(135deg, #1976d208 0%, #0050ac08 100%)' }}>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, mt: 2 }}>
       {[{ done: true, label: 'Connect your Twitter account' }, { done: true, label: 'Set content scraping preferences' }, { done: true, label: 'Customize your tweet templates' }, { done: false, label: 'Launch your automated tweeting campaign' }].map((s, i) => (
