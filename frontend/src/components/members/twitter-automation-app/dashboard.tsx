...
export function MembersDashboardPage() {
 const stats = [
  { label: 'Tweets Scheduled', value: '35', change: '+12%', icon: <Visibility />, color: COLORS.primary },
  { label: 'Followers Gained', value: '250', change: '+5%', icon: <People />, color: COLORS.blue },
  { label: 'Engagement Rate', value: '3.2%', change: '+0.5%', icon: <TrendingUp />, color: COLORS.success },
  { label: 'Opt-ins to E-book', value: '150', change: 'Top 10%', icon: <Star />, color: COLORS.warning },
 ];

 const recentActivity = [
  { title: 'Content Scraped', desc: 'Daily content has been scraped successfully.', time: '2 min ago', color: COLORS.success, icon: <People /> },
  { title: 'Tweets Posted', desc: 'Scheduled tweets have been posted to your timeline.', time: '1 hour ago', color: COLORS.warning, icon: <EmojiEvents /> },
  { title: 'Autoresponses Sent', desc: 'Autoresponses dispatched to user replies.', time: '3 hours ago', color: COLORS.primary, icon: <Bolt /> },
  { title: 'E-book Links Sent', desc: 'Links to your e-book have been shared with engaged users.', time: '5 hours ago', color: COLORS.blue, icon: <Star /> },
 ];
  ...
  <Box>
   <Paper sx={heroSx}>
    ...
    <Box sx={{ position: 'relative', zIndex: 1 }}>
     ...
     <Typography variant="body1" sx={{ opacity: 0.85, maxWidth: 500 }}>Track your Twitter engagement and automate your posts effortlessly.</Typography>
    </Box>
   </Paper>
    ...
    <Grid item xs={12} md={4}>
     ...
     <Paper sx={{ ...sectionSx, mt: 3, background: 'linear-gradient(135deg, #1976d208 0%, #0050ac08 100%)' }}>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, mt: 2 }}>
       {[{ done: true, label: 'Connect your Twitter account' }, { done: true, label: 'Set your content preferences' }, { done: true, label: 'Review the tweet schedule' }, { done: false, label: 'Launch your automated campaign' }].map((s, i) => (
