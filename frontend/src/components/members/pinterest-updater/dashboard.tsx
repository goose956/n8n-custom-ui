...
export function MembersDashboardPage() {
 const stats = [
  { label: 'Daily Pins Posted', value: '150', change: '+5%', icon: <Visibility />, color: COLORS.primary },
  { label: 'New Followers', value: '300', change: '+10%', icon: <People />, color: COLORS.blue },
  { label: 'Engagement Rate', value: '7.5%', change: '+2%', icon: <TrendingUp />, color: COLORS.success },
  { label: 'Saved Pins', value: '1,200', change: 'Top 30%', icon: <Star />, color: COLORS.warning },
 ];

 const recentActivity = [
  { title: 'Pinned New Content', desc: 'Posted 10 new pins today.', time: '2 min ago', color: COLORS.success, icon: <People /> },
  { title: 'Scheduled Pins', desc: '23 pins scheduled for this week.', time: '1 hour ago', color: COLORS.warning, icon: <EmojiEvents /> },
  { title: 'AI Replies Sent', desc: 'Responded to 15 messages automatically.', time: '3 hours ago', color: COLORS.primary, icon: <Bolt /> },
  { title: 'Data Scraped', desc: 'Successfully scraped trending content.', time: '5 hours ago', color: COLORS.blue, icon: <Star /> },
 ];
  ...
  <Box>
   <Paper sx={heroSx}>
    ...
    <Box sx={{ position: 'relative', zIndex: 1 }}>
     ...
     <Typography variant="body1" sx={{ opacity: 0.85, maxWidth: 500 }}>Effortlessly track and manage your Pinterest performance.</Typography>
    </Box>
   </Paper>
    ...
    <Grid item xs={12} md={4}>
     ...
     <Paper sx={{ ...sectionSx, mt: 3, background: 'linear-gradient(135deg, #1976d208 0%, #0050ac08 100%)' }}>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, mt: 2 }}>
       {[{ done: true, label: 'Connect your Pinterest account' }, { done: true, label: 'Set up scraping parameters' }, { done: true, label: 'Schedule your first batch of pins' }, { done: false, label: 'Review and customize auto-reply messages' }].map((s, i) => (
