...
export function MembersDashboardPage() {
 const stats = [
  { label: 'Total Views', value: '10,500', change: '+15%', icon: <Visibility />, color: COLORS.primary },
  { label: 'Subscribers Gained', value: '250', change: '+20%', icon: <People />, color: COLORS.blue },
  { label: 'Videos Transcribed', value: '35', change: '+10%', icon: <TrendingUp />, color: COLORS.success },
  { label: 'Engagement Rate', value: '75%', change: 'Top 5%', icon: <Star />, color: COLORS.warning },
 ];

 const recentActivity = [
  { title: 'Created New Script', desc: 'Generated a YouTube script for your next video.', time: '2 min ago', color: COLORS.success, icon: <People /> },
  { title: 'Scraped Emails', desc: 'Gathered contact details of top YouTube creators.', time: '1 hour ago', color: COLORS.warning, icon: <EmojiEvents /> },
  { title: 'Tracked View Changes', desc: 'Monitored the latest view count fluctuations.', time: '3 hours ago', color: COLORS.primary, icon: <Bolt /> },
  { title: 'Designed Thumbnail', desc: 'Created an eye-catching thumbnail for your video.', time: '5 hours ago', color: COLORS.blue, icon: <Star /> },
 ];
  ...
  <Box>
   <Paper sx={heroSx}>
    ...
    <Box sx={{ position: 'relative', zIndex: 1 }}>
     ...
     <Typography variant="body1" sx={{ opacity: 0.85, maxWidth: 500 }}>Track and optimize your YouTube channel performance effortlessly.</Typography>
    </Box>
   </Paper>
    ...
    <Grid item xs={12} md={4}>
     ...
     <Paper sx={{ ...sectionSx, mt: 3, background: 'linear-gradient(135deg, #1976d208 0%, #0050ac08 100%)' }}>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, mt: 2 }}>
       {[{ done: true, label: 'Connect your YouTube account' }, { done: true, label: 'Generate your first script' }, { done: true, label: 'Scrape emails for outreach' }, { done: false, label: 'Design your first thumbnail' }].map((s, i) => (
