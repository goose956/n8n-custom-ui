...
export function MembersDashboardPage() {
  ...
  const [quickLinks, setQuickLinks] = useState<Array<any>>([]);

  useEffect(() => {
    // Fetch data for stats, recentActivity and quickLinks
    // Assuming fetchData is an async function to fetch data
    const fetchData = async () => {
      // Example data fetching logic
      const statsData = [
        { label: 'Messages Sent', value: 250, change: '+10%', icon: <Visibility />, color: COLORS.primary },
        { label: 'Responses Received', value: 75, change: '+15%', icon: <People />, color: COLORS.blue },
        { label: 'Leads Generated', value: 20, change: '+5%', icon: <TrendingUp />, color: COLORS.success },
        { label: 'Conversion Rate', value: 0.08, change: 'Top 5%', icon: <Star />, color: COLORS.warning },
      ];

      const recentActivityData = [
        { title: 'New Outreach Campaign', desc: 'Launch a personalized DM outreach campaign.', time: '2 min ago', color: COLORS.success, icon: <People /> },
        { title: 'AI Responses Ready', desc: 'Review AI-generated replies for your conversations.', time: '1 hour ago', color: COLORS.warning, icon: <EmojiEvents /> },
        { title: 'Profile Insights', desc: 'Get insights on your target profiles.', time: '3 hours ago', color: COLORS.primary, icon: <Bolt /> },
        { title: 'Performance Overview', desc: 'Check the effectiveness of your messaging strategy.', time: '5 hours ago', color: COLORS.blue, icon: <Star /> },
      ];
    ...
    <Box>
      <Paper sx={heroSx}>
        ...
        <Box sx={{ position: 'relative', zIndex: 1 }}>
          ...
          <Typography variant="body1" sx={{ opacity: 0.85, maxWidth: 500 }}>Track your outreach efforts and watch your connections grow</Typography>
        </Box>
      </Paper>
        ...
        <Grid item xs={12} md={4}>
          ...
          <Paper sx={{ ...sectionSx, mt: 3, background: 'linear-gradient(135deg, #1976d208 0%, #0050ac08 100%)' }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, mt: 2 }}>
              {[{ done: true, label: 'Create your profile' }, { done: true, label: 'Integrate your LinkedIn account' }, { done: true, label: 'Set up your outreach campaign' }, { done: false, label: 'Analyze your results' }].map((s, i) => (
