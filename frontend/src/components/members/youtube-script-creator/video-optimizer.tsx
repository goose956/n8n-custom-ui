...
import {
  Box, Typography, Paper, Button, Grid, Card, CardContent, Chip, Avatar, LinearProgress,
  Divider, Skeleton, Tooltip, IconButton
} from '@mui/material';
...
export function MembersVideoOptimizerPage() {
  const [stats, setStats] = useState<{ totalScripts: number, engagements: number, seoScore: number }>({
    totalScripts: 123,
    engagements: 456,
    seoScore: 78
  });
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Box sx={{ mb: 4, background: 'linear-gradient(135deg, #1976d2 0%, #5147ad 100%)', borderRadius: 3, p: 3, boxShadow: '0 2px 12px rgba(0,0,0,0.06)', color: '#fff' }}>
        <Typography variant="h5" sx={{ mb: 1.5, display: 'flex', alignItems: 'center', gap: 1 }}><BarChartIcon /> Video Optimization Dashboard</Typography>
        <Typography variant="body1">Enhance your YouTube videos for better viewer engagement and higher SEO rankings with our AI-powered tools.</Typography>
      </Box>

      <Grid container spacing={3}>
        <Grid item xs={12} sm={6} md={4}>
          <Card sx={{ boxShadow: '0 4px 16px rgba(0,0,0,0.1)', borderRadius: 16, '&:hover': { transform: 'translateY(-2px)', transition: '0.2s' } }}>
            <CardContent sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', background: 'linear-gradient(90deg, #1976d2 0%, #5147ad 100%)', color: '#fff' }}>
              <Typography variant="h6" sx={{ display: 'flex', alignItems: 'center', gap: 1 }}><TagIcon /> SEO Score</Typography>
              <Typography variant="h4" sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>{stats.seoScore} <TrendingUpIcon color="inherit" /></Typography>
              <Chip label="Trending Up" sx={{ bgcolor: 'rgba(255,255,255,0.2)', mt: 1, color: 'white' }} />
            </CardContent>
          </Card>
        </Grid>
        <Grid item xs={12} sm={6} md={4}>
          <Card sx={{ boxShadow: '0 4px 16px rgba(0,0,0,0.1)', borderRadius: 16, '&:hover': { transform: 'translateY(-2px)', transition: '0.2s' } }}>
            <CardContent sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', background: 'linear-gradient(90deg, #00bcd4 0%, #1976d2 100%)', color: '#fff' }}>
              <Typography variant="h6" sx={{ display: 'flex', alignItems: 'center', gap: 1 }}><PlayArrowIcon /> Total Engagements</Typography>
              <Typography variant="h4" sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>{stats.engagements} <TrendingDownIcon color="inherit" /></Typography>
              <Chip label="Needs Improvement" sx={{ bgcolor: 'rgba(255,255,255,0.2)', mt: 1, color: 'white' }} />
            </CardContent>
          </Card>
        </Grid>
        <Grid item xs={12} sm={6} md={4}>
          <Card sx={{ boxShadow: '0 4px 16px rgba(0,0,0,0.1)', borderRadius: 16, '&:hover': { transform: 'translateY(-2px)', transition: '0.2s' } }}>
            <CardContent sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', background: 'linear-gradient(90deg, #f39c12 0%, #e74c3c 100%)', color: '#fff' }}>
              <Typography variant="h6" sx={{ display: 'flex', alignItems: 'center', gap: 1 }}><EditIcon /> Scripts Generated</Typography>
              <Typography variant="h4" sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>{stats.totalScripts} <TrendingUpIcon color="inherit" /></Typography>
              <Chip label="On Track" sx={{ bgcolor: 'rgba(255,255,255,0.2)', mt: 1, color: 'white' }} />
            </CardContent>
          </Card>
        </Grid>
      </Grid>
      ...
      <Box sx={{ mt: 5 }}>
        <SkillWidget placeholder="Describe your optimization needs..." title="YouTube Video Optimization AI" />
      </Box>
    </Box>
  );
}