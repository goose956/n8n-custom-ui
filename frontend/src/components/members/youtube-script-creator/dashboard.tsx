...
export function MembersDashboardPage() {
 const stats = [
  { label: 'Scripts Generated', value: '150', change: '+20%', icon: <Visibility />, color: COLORS.primary },
  { label: 'Emails Scraped', value: '1,200', change: '+15%', icon: <People />, color: COLORS.blue },
  { label: 'Transcriptions Completed', value: '300', change: '+5%', icon: <TrendingUp />, color: COLORS.success },
  { label: 'Thumbnails Designed', value: '75', change: 'Top 10%', icon: <Star />, color: COLORS.warning },
 ];

 const recentActivity = [
  { title: 'Create New Script', desc: 'Start crafting engaging YouTube scripts in minutes.', time: '2 min ago', color: COLORS.success, icon: <People /> },
  { title: 'Scrape Emails', desc: 'Gather potential leads for collaboration effectively.', time: '1 hour ago', color: COLORS.warning, icon: <EmojiEvents /> },
  { title: 'Generate Transcriptions', desc: 'Convert your video audio into text seamlessly.', time: '3 hours ago', color: COLORS.primary, icon: <Bolt /> },
  { title: 'Design Thumbnails', desc: 'Make eye-catching thumbnails to attract viewers.', time: '5 hours ago', color: COLORS.blue, icon: <Star /> },
 ];
  ...
  <Box>
   <Paper sx={heroSx}>
    ...
    <Box sx={{ position: 'relative', zIndex: 1 }}>
     ...
     <Typography variant="body1" sx={{ opacity: 0.85, maxWidth: 500 }}>Track your progress in creating compelling YouTube content effortlessly.</Typography>
    </Box>
   </Paper>
    ...
    <Grid item xs={12} md={4}>
     ...
     <Paper sx={{ ...sectionSx, mt: 3, background: 'linear-gradient(135deg, #1976d208 0%, #0050ac08 100%)' }}>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, mt: 2 }}>
       {[{ done: true, label: 'Sign up for an account' }, { done: true, label: 'Set up your channel information' }, { done: true, label: 'Explore the script templates' }, { done: false, label: 'Start generating scripts' }].map((s, i) => (
