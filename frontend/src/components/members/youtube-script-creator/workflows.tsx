...
function RichOutput({ text }: { text: string }) {
 ...
 function inlineFormat(str: string): React.ReactNode[] {
  ...
  while ((m = re.exec(str)) !== null) {
   ...
   else if (m[5]) { const href = m[6].startsWith('/') ? API_BASE + m[6] : m[6]; out.push(<a key={idx++} href={href} target="_blank" rel="noopener" style={{ color: '#1976d2' }}>{m[5]}</a>); }
    ...
    <Box sx={{ position: 'relative', zIndex: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
     <Box>
      <Typography variant="h4" sx={{ fontWeight: 800, display: 'flex', alignItems: 'center', gap: 1 }}>
       <Schedule /> YouTube script creator Workflows
      </Typography>
      <Typography sx={{ opacity: 0.85, mt: 0.5 }}>Automate the creation of scripts and thumbnails on a schedule to keep your content flow consistent.</Typography>
     </Box>
