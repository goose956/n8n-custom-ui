...
export function MembersDashboardPage() {
 const stats = [
  { label: 'Connection Requests Sent', value: '120', change: '+15%', icon: <Visibility />, color: COLORS.primary },
  { label: 'Responses Received', value: '45', change: '+20%', icon: <People />, color: COLORS.blue },
  { label: 'Auto-Replies Sent', value: '30', change: '+10%', icon: <TrendingUp />, color: COLORS.success },
  { label: 'Conversion Rate', value: '37%', change: 'Top 15%', icon: <Star />, color: COLORS.warning },
 ];

 const recentActivity = [
  { title: 'Sent Direct Messages', desc: 'You sent 10 personalized messages today.', time: '2 min ago', color: COLORS.success, icon: <People /> },
  { title: 'Profile Scanned', desc: 'AI scanned 25 new profiles for potential outreach.', time: '1 hour ago', color: COLORS.warning, icon: <EmojiEvents /> },
  { title: 'Follow-up Responses', desc: 'Auto-replies drafted for 5 follow-up messages.', time: '3 hours ago', color: COLORS.primary, icon: <Bolt /> },
  { title: 'Connection Requests Accepted', desc: 'You gained 8 new connections this week.', time: '5 hours ago', color: COLORS.blue, icon: <Star /> },
 ];
  ...
  <Box>
   <Paper sx={heroSx}>
    ...
    <Box sx={{ position: 'relative', zIndex: 1 }}>
     ...
     <Typography variant="body1" sx={{ opacity: 0.85, maxWidth: 500 }}>Track your LinkedIn outreach performance effortlessly.</Typography>
    </Box>
   </Paper>
    ...
    <Grid item xs={12} md={4}>
     ...
     <Paper sx={{ ...sectionSx, mt: 3, background: 'linear-gradient(135deg, #1976d208 0%, #0050ac08 100%)' }}>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, mt: 2 }}>
       {[{ done: true, label: 'Connect your LinkedIn account' }, { done: true, label: 'Set up your automated messaging' }, { done: true, label: 'Customize your AI reply templates' }, { done: false, label: 'Start your outreach campaign' }].map((s, i) => (
