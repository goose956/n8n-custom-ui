...
export function MembersContactPage() {
 ...
 const handleSubmit = async (e: React.FormEvent) => {
  ...
  try {
   const res = await fetch(API + '/api/contact', {
    ...
    body: JSON.stringify({ ...form, app_id: 13 }),
   });
 ...
 const infoCards = [
  { icon: <Chat />, title: 'Email Support', desc: 'Available Monday to Friday, 9 AM - 5 PM EST.', color: COLORS.primary },
  { icon: <AccessTime />, title: 'Live Chat', desc: 'Available during business hours for real-time assistance.', color: COLORS.blue },
  { icon: <SupportAgent />, title: 'Knowledge Base', desc: '24/7 access to articles and guides for self-service support.', color: COLORS.success },
 ];
  ...
  <Box sx={{ maxWidth: 800, mx: 'auto' }}>
   <Paper sx={heroSx}>
    ...
    <Box sx={{ position: 'relative', zIndex: 1 }}>
     ...
     <Typography variant="body1" sx={{ opacity: 0.85, mt: 0.5 }}>Need assistance? Our team is here to help you succeed!</Typography>
    </Box>
   </Paper>
