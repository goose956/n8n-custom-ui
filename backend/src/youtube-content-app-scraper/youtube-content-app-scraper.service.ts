...
@Injectable()
export class YoutubeContentAppScraperService {
  private readonly logger = new Logger(YoutubeContentAppScraperService.name);
  ...
  async runScraper(request: ScrapeRequest): Promise<ScrapeResponse> {
    try {
      ...
      const data = this.db.readSync();
      if (!data.youtubeContentAppScraperRuns) data.youtubeContentAppScraperRuns = {};

      data.youtubeContentAppScraperRuns[runData.id] = {
        id: runData.id,
  ...
  async getStatus(runId: string): Promise<StatusResponse> {
    try {
      ...
      // Update local DB
      const data = this.db.readSync();
      if (data.youtubeContentAppScraperRuns?.[runId]) {
        data.youtubeContentAppScraperRuns[runId].status = runData.status;
        data.youtubeContentAppScraperRuns[runId].updatedAt = new Date().toISOString();
        if (runData.finishedAt) data.youtubeContentAppScraperRuns[runId].finishedAt = runData.finishedAt;
        this.db.writeSync(data);
      }
  ...
  async getResults(runId: string): Promise<ResultsResponse> {
    try {
      const data = this.db.readSync();
      const runInfo = data.youtubeContentAppScraperRuns?.[runId];
      ...
      if (!datasetId) {
        ...
        data.youtubeContentAppScraperRuns[runId].defaultDatasetId = datasetId;
        this.db.writeSync(data);
      }
