...
export class PreviewService implements OnModuleDestroy {
  ...
  async start(req: {
    ...
    await server.listen();
    this.logger.warn(`[DEBUG] ✅ Vite server.listen() completed — http://localhost:${port}`);
...
try {
  ...
  console.error('[PREVIEW DEBUG] ❌ FAILED to import component module:', err.message, err.stack);
...
function _resolve(): React.ComponentType<any> {
  ...
  console.error('[PREVIEW DEBUG] ❌ No renderable component found! Module keys:', Object.keys(_M));
...
try {
  ...
  console.log('[PREVIEW DEBUG] ✅ render() called successfully');
} catch (err: any) {
  console.error('[PREVIEW DEBUG] ❌ Render error:', err.message, err.stack);
