...
class DatabaseMigration {
  ...
  writeNewDB(newDB: NewDB): void {
    try {
      ...
      console.log(`✅ New database structure written to ${this.dbPath}`);
    } catch (error) {
  ...
  execute(): void {
    ...
    try {
      ...
      console.log(`✅ Read ${Object.keys(legacyDB).length} data collections\n`);
      ...
      console.log(`✅ Created ${newDB.apps.length} app(s)`);
      console.log(`✅ Created ${newDB.pages.length} page(s)`);
      console.log(`✅ Migrated ${newDB.workflows.length} workflow(s)\n`);
