...
@Injectable()
export class TwitterAutomationAppScraperService {
  private readonly logger = new Logger(TwitterAutomationAppScraperService.name);
  ...
  async runScraper(request: ScrapeRequest): Promise<ScrapeResponse> {
    try {
      ...
      const data = this.db.readSync();
      if (!data.twitterAutomationAppScraperRuns) data.twitterAutomationAppScraperRuns = {};

      data.twitterAutomationAppScraperRuns[runData.id] = {
        id: runData.id,
  ...
  async getStatus(runId: string): Promise<StatusResponse> {
    try {
      ...
      // Update local DB
      const data = this.db.readSync();
      if (data.twitterAutomationAppScraperRuns?.[runId]) {
        data.twitterAutomationAppScraperRuns[runId].status = runData.status;
        data.twitterAutomationAppScraperRuns[runId].updatedAt = new Date().toISOString();
        if (runData.finishedAt) data.twitterAutomationAppScraperRuns[runId].finishedAt = runData.finishedAt;
        this.db.writeSync(data);
      }
  ...
  async getResults(runId: string): Promise<ResultsResponse> {
    try {
      const data = this.db.readSync();
      const runInfo = data.twitterAutomationAppScraperRuns?.[runId];
      ...
      if (!datasetId) {
        ...
        data.twitterAutomationAppScraperRuns[runId].defaultDatasetId = datasetId;
        this.db.writeSync(data);
      }
