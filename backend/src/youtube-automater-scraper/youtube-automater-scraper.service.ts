...
@Injectable()
export class YoutubeAutomaterScraperService {
  private readonly logger = new Logger(YoutubeAutomaterScraperService.name);
  ...
  async runScraper(request: ScrapeRequest): Promise<ScrapeResponse> {
    try {
      ...
      const data = this.db.readSync();
      if (!data.youtubeAutomaterScraperRuns) data.youtubeAutomaterScraperRuns = {};

      data.youtubeAutomaterScraperRuns[runData.id] = {
        id: runData.id,
  ...
  async getStatus(runId: string): Promise<StatusResponse> {
    try {
      ...
      // Update local DB
      const data = this.db.readSync();
      if (data.youtubeAutomaterScraperRuns?.[runId]) {
        data.youtubeAutomaterScraperRuns[runId].status = runData.status;
        data.youtubeAutomaterScraperRuns[runId].updatedAt = new Date().toISOString();
        if (runData.finishedAt) data.youtubeAutomaterScraperRuns[runId].finishedAt = runData.finishedAt;
        this.db.writeSync(data);
      }
  ...
  async getResults(runId: string): Promise<ResultsResponse> {
    try {
      const data = this.db.readSync();
      const runInfo = data.youtubeAutomaterScraperRuns?.[runId];
      ...
      if (!datasetId) {
        ...
        data.youtubeAutomaterScraperRuns[runId].defaultDatasetId = datasetId;
        this.db.writeSync(data);
      }
