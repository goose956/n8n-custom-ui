...
@Injectable()
export class LinkedinPowerhouseScraperService {
  private readonly logger = new Logger(LinkedinPowerhouseScraperService.name);
  ...
  async runScraper(request: ScrapeRequest): Promise<ScrapeResponse> {
    try {
      ...
      const data = this.db.readSync();
      if (!data.linkedinPowerhouseScraperRuns) data.linkedinPowerhouseScraperRuns = {};

      data.linkedinPowerhouseScraperRuns[runData.id] = {
        id: runData.id,
  ...
  async getStatus(runId: string): Promise<StatusResponse> {
    try {
      ...
      // Update local DB
      const data = this.db.readSync();
      if (data.linkedinPowerhouseScraperRuns?.[runId]) {
        data.linkedinPowerhouseScraperRuns[runId].status = runData.status;
        data.linkedinPowerhouseScraperRuns[runId].updatedAt = new Date().toISOString();
        if (runData.finishedAt) data.linkedinPowerhouseScraperRuns[runId].finishedAt = runData.finishedAt;
        this.db.writeSync(data);
      }
  ...
  async getResults(runId: string): Promise<ResultsResponse> {
    try {
      const data = this.db.readSync();
      const runInfo = data.linkedinPowerhouseScraperRuns?.[runId];
      ...
      if (!datasetId) {
        ...
        data.linkedinPowerhouseScraperRuns[runId].defaultDatasetId = datasetId;
        this.db.writeSync(data);
      }
