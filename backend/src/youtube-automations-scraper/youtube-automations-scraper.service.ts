...
@Injectable()
export class YoutubeAutomationsScraperService {
  private readonly logger = new Logger(YoutubeAutomationsScraperService.name);
  ...
  async runScraper(request: ScrapeRequest): Promise<ScrapeResponse> {
    try {
      ...
      const data = this.db.readSync();
      if (!data.youtubeAutomationsScraperRuns) data.youtubeAutomationsScraperRuns = {};

      data.youtubeAutomationsScraperRuns[runData.id] = {
        id: runData.id,
  ...
  async getStatus(runId: string): Promise<StatusResponse> {
    try {
      ...
      // Update local DB
      const data = this.db.readSync();
      if (data.youtubeAutomationsScraperRuns?.[runId]) {
        data.youtubeAutomationsScraperRuns[runId].status = runData.status;
        data.youtubeAutomationsScraperRuns[runId].updatedAt = new Date().toISOString();
        if (runData.finishedAt) data.youtubeAutomationsScraperRuns[runId].finishedAt = runData.finishedAt;
        this.db.writeSync(data);
      }
  ...
  async getResults(runId: string): Promise<ResultsResponse> {
    try {
      const data = this.db.readSync();
      const runInfo = data.youtubeAutomationsScraperRuns?.[runId];
      ...
      if (!datasetId) {
        ...
        data.youtubeAutomationsScraperRuns[runId].defaultDatasetId = datasetId;
        this.db.writeSync(data);
      }
