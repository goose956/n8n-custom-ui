...
@Injectable()
export class PinterestUpdaterScraperService {
  private readonly logger = new Logger(PinterestUpdaterScraperService.name);
  ...
  async runScraper(request: ScrapeRequest): Promise<ScrapeResponse> {
    try {
      ...
      const data = this.db.readSync();
      if (!data.pinterestUpdaterScraperRuns) data.pinterestUpdaterScraperRuns = {};

      data.pinterestUpdaterScraperRuns[runData.id] = {
        id: runData.id,
  ...
  async getStatus(runId: string): Promise<StatusResponse> {
    try {
      ...
      // Update local DB
      const data = this.db.readSync();
      if (data.pinterestUpdaterScraperRuns?.[runId]) {
        data.pinterestUpdaterScraperRuns[runId].status = runData.status;
        data.pinterestUpdaterScraperRuns[runId].updatedAt = new Date().toISOString();
        if (runData.finishedAt) data.pinterestUpdaterScraperRuns[runId].finishedAt = runData.finishedAt;
        this.db.writeSync(data);
      }
  ...
  async getResults(runId: string): Promise<ResultsResponse> {
    try {
      const data = this.db.readSync();
      const runInfo = data.pinterestUpdaterScraperRuns?.[runId];
      ...
      if (!datasetId) {
        ...
        data.pinterestUpdaterScraperRuns[runId].defaultDatasetId = datasetId;
        this.db.writeSync(data);
      }
